# LangGraph 的可执行图：从 Runnable 到 CompiledStateGraph - Quiz
...
1. 用 5 句话以内解释 `StateGraph, CompiledStateGraph` 与本课 claim 的关系。
2. 从 `ev-test-state-basic` 和 `ev-test-runnable` 各选一个测试，说明它们分别证明了什么。
